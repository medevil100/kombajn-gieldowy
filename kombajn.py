...
st.set_page_config(page_title="NEON MASTER v93 OMNI", page_icon="🏆", layout="wide")
...
st.markdown("""
    ...
    .status-buy { color: #00ff88; text-shadow: 0 0 10px #00ff88; font-weight: bold; border: 2px solid #00ff88; padding: 5px 15px; border-radius: 10px; text-transform: uppercase; }
    .status-sell { color: #ff0055; text-shadow: 0 0 10px #ff0055; font-weight: bold; border: 2px solid #ff0055; padding: 5px 15px; border-radius: 10px; text-transform: uppercase; }
    .status-hold { color: #58a6ff; text-shadow: 0 0 10px #58a6ff; font-weight: bold; border: 2px solid #58a6ff; padding: 5px 15px; border-radius: 10px; text-transform: uppercase; }
...
def get_data(symbol):
    try:
        ...
        if rsi < 1: return None # Filtr śmieciowych danych
...
def run_ai(d, key):
    ...
    try:
        ...
        prompt = (f"Trader PRO. Analiza {d['symbol']} cena {d['price']}. RSI:{d['rsi']:.1f}, MA50:{d['ma50']:.1f}, Pivot:{d['pp']:.2f}. "
                  f"Ostatnie ceny:\n{d['d1_hist']}\n"
...
# --- 4. PANEL BOCZNY ---
with st.sidebar:
    st.title("🏆 MASTER v93 OMNI")
    key = st.secrets.get("OPENAI_API_KEY") or st.text_input("OpenAI Key", type="password")
    cap = st.number_input("💵 Kapitał:", value=float(st.session_state.risk_cap))
    risk_p = st.slider("Ryzyko %", 0.1, 5.0, float(st.session_state.risk_pct))
    ...
    if st.button("🚀 INICJUJ SYSTEM"):
        ...
        st.rerun()
    st_autorefresh(interval=60000, key="v93_ref")
        ...
        with c3:
            if ai:
                ...
                try:
                    ...
                    shares = int((cap * (risk_p/100)) / diff) if diff > 0 else 0
