...
def get_analysis(symbol):
    try:
        ...
        # Próba pobrania Bid/Ask (działa głównie dla akcji US w sesji)
        try:
            info = t.info
        ...
        # Wskaźniki
        sma50 = float(d1['Close'].rolling(50).mean().iloc[-1])
        sma200 = float(d1['Close'].rolling(200).mean().iloc[-1])
        yearly_high = float(d1['High'].max())
        yearly_low = float(d1['Low'].min())
        ...
        return {
            ...
            "df": h1, "change": float(((price - cp) / cp * 100))
        }
...
if all_data:
    st.subheader("🔥 TOP SYGNAŁY (Ranking RSI)")
    ...
    for d in all_data:
        with st.container():
            ...
            with c2:
                # Świece + linie poziomów rocznych
            ...
            with c3:
                ...
                if api_key and st.button(f"🧠 AI STRATEGIA", key=f"ai_{d['symbol']}"):
                    ...
                    prompt = (f"Analiza {d['symbol']}: Cena {d['price']}, RSI {d['rsi']:.1f}, "
                              f"SMA200 {d['sma200']:.2f}, High12m {d['y_high']}, Low12m {d['y_low']}. "
                              f"Daj krótki plan: Buy/Sell/Hold, SL i TP.")
