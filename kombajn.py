...
<style>
    ...
    .top-tile { background: #161b22; border-radius: 12px; padding: 12px; text-align: center; min-height: 100px; border-bottom: 4px solid #58a6ff; }
...
def get_ai_analysis(d, key):
    try:
        ...
        prompt = (f"Przeanalizuj technicznie {d['symbol']} cena {d['price']}. RSI:{d['rsi']:.1f}, Trendy:{d['trends']}. "
                  f"Odpowiedz po POLSKU, konkretnie. Zwróć JSON: {{\"w\": \"KUP\" lub \"CZEKAJ\", \"sl\": {round(d['price']*0.94, 2)}, \"tp\": {round(d['price']*1.12, 2)}, \"u\": \"3 punkty analizy\"}}")
        ...
        return {"w": "BŁĄD", "u": "Nie udało się połączyć z API OpenAI."}
...
if data_list:
    # --- TOP 10 RADAR (PRZYWRÓCONE DANE I KOLORY) ---
    ...
    for i, r in enumerate(sorted_top):
        with all_cols[i]:
            # Kolor kafelka zależny od RSI (im niższe, tym bardziej zielone)
            tile_col = "#00ff88" if r['rsi'] < 30 else "#58a6ff" if r['rsi'] < 50 else "#ff4b4b"
            st.markdown(f"""
                <div class="top-tile" style="border-bottom: 4px solid {tile_col}">
                    <b>{r["symbol"]}</b><br>
                    <span style="color:{tile_col}">{r["price"]:.2f} USD</span><br>
    ...
    for d in data_list:
        ai = st.session_state.ai_memo.get(d['symbol'], {"w": "CZEKAJ", "u": "Kliknij przycisk, aby pobrać analizę."})
        ...
        with c3:
            ...
            if st.button(f"ZATWIERDŹ ZAKUP", key=f"buy_{d['symbol']}"):
                if buy_qty > 0:
                    st.session_state.risk_cap_pln -= total_cost
                    save_cap(st.session_state.risk_cap_pln)
                    st.success("Zakupiono!")
                    st.rerun()
