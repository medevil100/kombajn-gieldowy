...
st.set_page_config(page_title="AI ALPHA GOLDEN v52 FINAL", page_icon="🚜", layout="wide")
...
st.markdown("""
    ...
    .main-card { 
        ...
        text-align: center; min-height: 680px; transition: 0.3s;
    ...
    .ai-box { 
        ...
        min-height: 100px; display: flex; align-items: center; line-height: 1.4; font-style: italic;
    }
...
def get_full_analysis(symbol):
    try:
        ...
        shares = int((st.session_state.risk_cap * (st.session_state.risk_pct / 100)) / (atr * 1.6)) if atr > 0 else 0
        ...
        if not news_data: news_data = [{"t": "Brak komunikatów", "u": "#"}]
...
st_autorefresh(interval=ref_val * 1000, key="v52_refresh")
...
if data_ready:
    st.subheader("🏆 TOP 10 RANKING RSI")
        ...
        for idx, d in enumerate(data_ready[i:i+5]):
            with cols[idx]:
                ...
                <div class="main-card" style="border: 2px solid {accent};">
                    ...
                    <div class="pos-calc">KUPNO: {d['shares']} szt.<br><small>{d['val']:.0f} PLN</small></div>
                    <div class="tech-grid">
                    ...
                    <div class="ai-box" id="ai_{d['s']}">Kliknij przycisk poniżej dla strategii AI</div>
...
st.markdown(f"<div style='text-align:center; color:#8b949e; margin-top:50px;'>v52.0 | {datetime.now().strftime('%H:%M:%S')}</div>", unsafe_allow_html=True)
