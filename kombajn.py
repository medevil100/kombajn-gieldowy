...
# Globalne odświeżanie (co 60 sekund)
st_autorefresh(interval=60 * 1000, key="global_ref")
...
def run_ai_analysis(d, key):
    try:
        client = OpenAI(api_key=key)
        prompt = (f"Analyze {d['symbol']} @ {d['price']}. RSI:{d['rsi']:.1f}, Trends:{d['trends']}. "
                  f"Be critical. Return JSON: {{\"w\": \"KUP/CZEKAJ\", \"sl\": {round(d['price']*0.94, 2)}, \"tp\": {round(d['price']*1.12, 2)}, \"u\": \"3-4 konkretne powody techniczne\"}}")
        resp = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "user", "content": prompt}], response_format={"type": "json_object"})
        return json.loads(resp.choices[0].message.content)
    except: return {"w": "CZEKAJ", "sl": 0, "tp": 0, "u": "Błąd AI."}
...
if data_list:
    ...
    for d in data_list:
        ai = run_ai_analysis(d, api_key) if api_key else {"w": "CZEKAJ", "u": "Brak klucza."}
        ...
        with c2:
            ...
            st.markdown(f'<div class="ai-full-box"><b>ANALIZA TECHNICZNA:</b><br>{ai["u"]}</div>', unsafe_allow_html=True)
        with c3:
            price_pln = d['ask'] * usd_pln_rate
            max_qty = int(st.session_state.risk_cap_pln / price_pln) if price_pln > 0 else 0
            
            # --- RUCHOMY SUWAK ILOŚCI ---
            ...
            if st.button(f"POTWIERDŹ ZAKUP {buy_qty} SZT.", key=f"buy_{d['symbol']}"):
                if buy_qty > 0:
            ...
            st.markdown(f"TP: <span style='color:#00ff88'>{ai.get('tp',0)}</span> | SL: <span style='color:#ff4b4b'>{ai.get('sl',0)}</span>", unsafe_allow_html=True)
