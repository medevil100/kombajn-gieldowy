...
def get_analysis(symbol):
    try:
        ...
        # Pobieranie danych 1h i 1d (szerszy zakres dla szczyt w/do?k w)
        h1 = t.history(period="10d", interval="1h")
        d1 = t.history(period="1y", interval="1d") # 1 rok dla ekstrem w
        ...
        # Wska?niki i Ekstrema (G rki i Do?ki)
...
if all_data:
    ...
    st.subheader("? TOP SYGNA?Y (Bid/Ask w rankingu)")
    ...
    for d in all_data:
        with st.container():
            ...
            with c1:
                ...
                st.markdown(f"""
                    <div style="margin-top:15px;">
                        ...
                        <div class="metric-row"><span style="color:#ff4b4b;">Do?ek 52t</span><b>{d['y_low']:.2f}</b></div>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with c2:
                ...
                # Linie szczyt w i do?k w na wykresie
            ...
            with c3:
                ...
                if api_key and st.button(f"? AI STRATEGIA", key=f"ai_{d['symbol']}"):
                    ...
                    # Wys?anie kompletnych danych historycznych do AI
                    prompt = (f"Przeanalizuj {d['symbol']}. Dane: Cena {d['price']}, Bid {d['bid']}, Ask {d['ask']}. "
                             f"RSI {d['rsi']:.1f}, SMA50 {d['sma50']:.2f}, SMA200 {d['sma200']:.2f}. "
                             f"Szczyt 12m: {d['y_high']}, Do?ek 12m: {d['y_low']}. "
                             f"Podaj konkretny werdykt i plan wej?cia/wyj?cia.")
