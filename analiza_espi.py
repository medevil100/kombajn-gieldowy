...
st.markdown("""
    ...
    .top-tile { background: #161b22; padding: 12px; border-radius: 10px; border: 1px solid #30363d; text-align: center; margin-bottom: 10px; min-height: 120px; }
...
def get_data(symbol):
    try:
        ...
        trend_class = "trend-up" if trend_val == "WZROST" else "trend-down"
        ...
        return {
            ...
            "trend": trend_val, "trend_class": trend_class,
...
if data_list:
    # --- TOP 10 SYGNAŁÓW (Naprawione na 2 rzędy) ---
        ...
        with cols1[i]:
            st.markdown(f"""
                <div class="top-tile">
                    ...
                    <span class="trend-label {d['trend_class']}">{d['trend']}</span>
                </div>
            """, unsafe_allow_html=True)
    ...
    if len(sorted_data) > 5:
        cols2 = st.columns(5)
        for i in range(5, min(10, len(sorted_data))):
            d = sorted_data[i]
            with cols2[i-5]:
                st.markdown(f"""
                    <div class="top-tile">
    ...
    for d in data_list:
        ...
        with c1:
            ...
            st.markdown(f"""
                ...
                <div class="metric-row"><span>Trend</span><b class="{d['trend_class']}">{d['trend']}</b></div>
            """, unsafe_allow_html=True)
            if api_key and st.button(f"🚀 DECYZJA AI", key=f"btn_{d['symbol']}"):
                client = OpenAI(api_key=api_key)
                prompt = f"Ticker: {d['symbol']}, Cena: {d['price']}, RSI: {d['rsi']:.1f}, Candle: {d['candle']}. Podaj: 1. DECYZJA, 2. TP/SL, 3. POWÓD (1 zdanie)."
                res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": "Jesteś traderem. Tylko konkrety."}, {"role": "user", "content": prompt}])
    ...
    st.error("Błąd pobierania danych. Sprawdź listę symboli.")
