...
# =================================================================
st.set_page_config(layout="wide", page_title="NEON MEGA-KOMBAJN ULTRA", page_icon="🚀")
...
st.markdown("""
    ...
    .signal-KUP { color: #39FF14; font-weight: bold; text-shadow: 0 0 10px #39FF14; border: 2px solid #39FF14; padding: 10px; border-radius: 8px; text-align: center; font-size: 1.3rem; }
    .signal-SPRZEDAJ { color: #FF3131; font-weight: bold; text-shadow: 0 0 10px #FF3131; border: 2px solid #FF3131; padding: 10px; border-radius: 8px; text-align: center; font-size: 1.3rem; }
    .signal-TRZYMAJ { color: #00FFFF; font-weight: bold; text-shadow: 0 0 10px #00FFFF; border: 2px solid #00FFFF; padding: 10px; border-radius: 8px; text-align: center; font-size: 1.3rem; }
    ...
    .val { color: #fff; font-weight: bold; font-size: 1.1rem; }
    </style>
...
st_autorefresh(interval=5 * 60 * 1000, key="mega_kombajn_v_final")

# =================================================================
# 3. SILNIK MATEMATYCZNY ULTRA (Pełna logika bez błędnych bibliotek)
# =================================================================
def get_mega_metrics(symbol):
    try:
        ...
        pressure = "KUPUJĄCY" if last > (open_p + last)/2 else "SPRZEDAJĄCY"
        ...
        return {
            ...
            "pat": pattern, "pres": pressure, "sig": ("KUP" if score >= 4 and rsi < 70 else "SPRZEDAJ" if score <= -3 else "TRZYMAJ")
        }
    except: return None
    ...
    for sym in tickers:
        ...
        with st.container():
            ...
            with c2:
                ...
                st.write(f"Trend (K|Ś|D): **{d['trends']}**")
                st.write(f"Trend Score: **{d['score']}**")
                st.write(f"Pivot Point: **{d['piv']:.2f}**")
                st.write(f"Szczyt 52T: **{d['h52']:.2f}**")
            ...
            with c4:
                ...
                if st.button(f"DIAGNOZA SYSTEMOWA 🤖", key=f"ai_{sym}"):
                    if client:
                        with st.spinner("ANALIZUJĘ..."):
                            prompt = f"Analiza {sym}: Cena {d['p']}, Score {d['score']}, RSI {d['rsi']:.1f}, Vol {d['vol']:.2f}x, ATR {d['atr']:.2f}, MACD Hist {d['macd_h']:.4f}, Świeca {d['pat']}, Presja {d['pres']}."
                            try:
                                resp = client.chat.completions.create(
                                    model="gpt-4o", 
                                    messages=[
                                        {"role": "system", "content": "Jesteś surowym algorytmem diagnostycznym. Zakaz lania wody. Zakaz definicji. Format: 1. TREND, 2. DYNAMIKA, 3. WERDYKT."},
                    ...
                    else: st.error("Brak klucza OpenAI w Secrets!")
