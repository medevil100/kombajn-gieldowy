...
st.set_page_config(
    page_title="CYBER‑DESK PRO ULTRA v4.2",
    page_icon="💠",
    layout="wide",
)
...
def detect_market(ticker: str) -> str:
    """
    AUTO-DETECT rynku na podstawie sufiksu tickera.
    Zwraca: 'GPW', 'LSE', 'XETRA', 'USA', 'OTHER'
    """
    t = ticker.upper()
    if t.endswith(".WA"):
        return "GPW"
    ...
    if t.endswith(".DE"):
        return "XETRA"
    # proste heurystyki ETF / indeksów
    if t.startswith("^"):
        return "INDEX"
    return "USA"

def normalize_ohlcv(df: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
    """
    Ujednolicenie danych OHLCV:
    - obsługa MultiIndex (GPW/ETF/indeksy)
    - dropna na Close
    - fillna(0) na Volume
    """
    if df.empty:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # zakładamy pierwszy instrument
        close = df["Close"].iloc[:, 0].dropna()
        vol = df["Volume"].iloc[:, 0].fillna(0)
    else:
...
def fetch_data(ticker: str, period: str, interval: str) -> tuple[pd.DataFrame, pd.Series, pd.Series]:
    """
    Pobiera dane z yfinance + stosuje normalize_ohlcv.
    Zwraca: (df, close, volume)
    """
...
# ============================================================
#   INDICATORS — ENGINE 3.2 (zero NaN, stabilny)
# ============================================================

def compute_indicators(close: pd.Series, volume: pd.Series):
    close = close.copy()
...
# ============================================================
#   MONITOR — RIGHT PANEL (AUTO‑HEIGHT, v4.2)
# ============================================================
...
def render_monitor(monitor_tickers, refresh_choice):
    ...
    for tck in tick_list:
        try:
            df, close, vol = fetch_data(tck, "5d", "1d")
...
# ============================================================
#   TRADING PANEL — LEFT + RIGHT COLUMNS (v4.2)
# ============================================================

def render_trading():
    st.markdown("## 📈 Kombajn Tradingowy — ULTRA Panel v4.2 (COMPACT‑MINI)")

    left, right = st.columns([0.7, 0.3])

    # ------------------------------------------------------------
    #   LEWA KOLUMNA — GŁÓWNY TICKER
    # ------------------------------------------------------------
    with left:
        ...
        if st.button("Pobierz dane i policz sygnały", use_container_width=True):
            ...
            df, close, vol = fetch_data(ticker, period, interval)
            ...
            # Wykres główny
            render_main_chart(ticker, df, ind)
