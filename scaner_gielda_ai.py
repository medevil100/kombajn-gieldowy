...
Masz dostęp do danych, które dostarcza aplikacja (ceny, wskaźniki, fundamenty, poziomy SL/TP).
...
AI_SYSTEM_DAY = """
Tryb: DAY TRADING / SCALPING.
Skupiasz się na:
- krótkoterminowym momentum,
- poziomach intraday,
- szybkim R/R,
- ryzyku wybicia SL w krótkim horyzoncie.
Odpowiedzi mają być krótkie, konkretne, nastawione na decyzję: wejście / brak wejścia / szybki TP/SL.
"""

AI_SYSTEM_SWING = """
Tryb: SWING TRADING (2–10 dni).
Skupiasz się na:
- strukturze trendu,
- kluczowych poziomach wsparcia/oporu,
- scenariuszach na kilka sesji,
- zarządzaniu pozycją w czasie.
Odpowiedzi mają być średniej długości, scenariuszowe.
"""

AI_SYSTEM_INVEST = """
Tryb: INWESTOR / FUNDAMENTALNY.
Skupiasz się na:
- jakości biznesu,
- relacji wyceny do fundamentów,
- stabilności trendu,
- dywidendzie, ryzyku długoterminowym.
Odpowiedzi mogą być dłuższe, ale nadal konkretne.
"""
...
if "tryb_ai" not in st.session_state:
    st.session_state["tryb_ai"] = "Hybrydowy"
...
st.sidebar.markdown("---")
st.sidebar.subheader("🧠 Tryb pracy AI")
tryb_ai = st.sidebar.radio(
    "Styl analizy:",
    ["Hybrydowy", "Day trading / scalping", "Swing trading", "Inwestor / fundamentalny"],
    index=0
)
st.session_state["tryb_ai"] = tryb_ai
...
def wyciagnij_ticker_z_tekstu(tekst: str) -> Optional[str]:
    raw = tekst.replace(",", " ").replace(":", " ").replace(";", " ").replace("?", " ").replace("!", " ")
...
def zbuduj_system_prompt() -> str:
    base = AI_SYSTEM_BASE
    if st.session_state["tryb_ai"] == "Day trading / scalping":
        return base + "\n" + AI_SYSTEM_DAY
    if st.session_state["tryb_ai"] == "Swing trading":
        return base + "\n" + AI_SYSTEM_SWING
    if st.session_state["tryb_ai"] == "Inwestor / fundamentalny":
        return base + "\n" + AI_SYSTEM_INVEST
    return base  # Hybrydowy – model sam dopasuje styl
...
"""
    system_prompt = zbuduj_system_prompt()
...
# ---------------------------------------------------------------------
with zakladki[1]:
    st.subheader("🔥 Raport inwestycyjny AI – tryb ULTRA 2.0")
...
# ---------------------------------------------------------------------
# ZAKŁADKA 5 – ASYSTENT AI 2.0
# ---------------------------------------------------------------------
with zakladki[4]:
    st.subheader("💬 Asystent AI 2.0 – rozmowa jak z analitykiem")
